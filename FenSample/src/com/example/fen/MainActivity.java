...
/**
 * 示例讲解：
 * new Fen(this).pop(view, "setImageBitmap").pop("CutBitmap").end("getBitmap", R.drawable.ic_launcher);
 * 从左到右，每一个pop将会使用反射找到对象和方法并压入一个方法栈中。
 * 当end发生时，将触发方法栈中的方法开始执行。
 * 
 * 因为是基于栈，所以方法的实际执行过程是从右至左。模拟顺序执行，不管是在异步线程还是主线程。
 * 
 * 示例的具体过程是：
 * 1：最先执行了activity对象下的getBitmap方法，提供给getBitmap方法的参数是R.drawable.ic_launcher，返回值是一张图片。
 * 2：将1方法中获取到的返回值（Bitmap）传递给activity对象下的CutBitmap方法。
...
public class MainActivity extends Activity {
	...
	public Bitmap CutBitmap(Bitmap bitmap) {
		//模仿切图过程，在背景线程中
		Log.d("demo", "CutBitmap 所在线程ID：" + Thread.currentThread().getId());
		try {
			Thread.sleep(1000);
	...
	public Bitmap getBitmap(int resId) {
		//模仿下载过程，在异步线程中
		Log.d("demo", "getBitmap 所在线程ID：" + Thread.currentThread().getId());
		try {
			Thread.sleep(5000);
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
